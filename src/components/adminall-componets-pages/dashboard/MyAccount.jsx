...
const MyAccount = () => {
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      ...
      try {
        ...
        const userResponse = await axios.get(`${BASE_URL}/api/users/profile/${id}`)
        setUserInfo(userResponse.data);
        // console.log('profile shows', userResponse.data)

        //fetch order history
        const transactionsResponse  = await axios.get(`${BASE_URL}/api/transactions/all`)
        ...
        setTransactions(formattedTransactions); //latest orders first 
      }catch(error) {
  ...
  return (
    ...
    <div className="my-account-container">
      ...
      <div className="balance-section">
        <div className="user-info">
          ...
          {userInfo.profileImage && (
            ...
            src={userInfo?.profileImage ? `${BASE_URL}/profileImage/${userInfo.profileImage}` : "https://i.pinimg.com/originals/78/71/71/78717192789208dfbf61261f4625da35.jpg"}
            alt="profile" className="profile-image" />
          )}
          <p className="user-name">{userInfo.name}</p>
          <p className="balance-text">Total Balance</p>
        {/* very important  */}
          <h3 className="balance-amount">₹{transactions.reduce((total, txn) => total + parseFloat(txn.amount.replace(/[^\d.]/g, "")), 0).toLocaleString("en-IN", {minimumFractionDigits: 2})}</h3>
        </div>
      ...
      <div className="transactions-section">
        ...
        src={
          transaction.user?.profileImage
            ? `${BASE_URL}/profileImage/${transaction.user.profileImage}`
      ...
      />
                <div>
                  <p className="transaction-name" >{transaction.user?.name}</p>
